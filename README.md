...
### 📊 Featured Projects
- **[3C: Clinical Code Collector for NICE/NHS](https://github.com/aydiny/clinical-code-collector)** — Agentic LangGraph pipeline (4-node stateful graph) combining LangChain RAG (ChromaDB, MMR retrieval) with GPT-4o-mini and deterministic NHS FHIR API search to automate SNOMED CT clinical code discovery. Hybrid LLM + deterministic design ensures full explainability and audit trail - grounding every code selection in exact NICE/QOF guideline evidence. Deployed on cloud at £0.01/query.
- **[Adversarial Multi-Agent Clinical Documentation System](https://github.com/aydiny/medbrain-squad)**  Fine-tuned MedGemma 1.5 4B with LoRA to convert doctor shorthand into hallucination-checked SOAP EHR records. Three-tier adversarial pipeline with PII scrubbing and independent Auditor agent - 100% local, GDPR-compliant, edge-deployable
- **[Topic Modelling of Gym Reviews for Actionable Strategies](https://github.com/aydiny/gym-customer-retention-nlp)** Leveraged Transformer models like BERTopic, DistilBert as well as classical NLP methods (LDA) to draw insights from customer experiences of a global gym company, through their reviews online. Integrated an LLM (Phi-4) to distil information and generate solutions for the critical topics. 
...
- **[Anomaly Detection](https://github.com/aydiny/predictive-maintenance-anomaly-detection)** - One-class SVM for monitoring a ship's engine to develop an early failure detection system
